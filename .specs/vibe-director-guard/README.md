...
4. Phase 4: Grounded Execution Dispatch (requires all six brief sections before any spawn; reconnaissance scouts bypass only implementation-specific gates after validation).
...
- `pi.on("tool_call")`: Monitors Director actions; tracks `read` calls; validates all six brief sections before any `vibe_spawn`; then permits scouts to bypass only implementation gates and checks reads, concrete paths, read intersection, and Master-TODO for implementation workers.
