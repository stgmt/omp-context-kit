...
const pluginName = packageManifest.name;
...
function hasPlugin(stdout) {
	try {
		...
		return Array.isArray(parsed.npm) && parsed.npm.some((plugin) => plugin?.name === pluginName);
	} catch {
		return false;
	}
}
...
for (const profile of await profileNames()) {
	...
	if (listed.status !== 0)
		throw new Error(`OMP plugin list failed for ${profile}: ${(listed.stderr || listed.stdout || "unknown error").trim()}`);
	if (!hasPlugin(listed.stdout)) {
		results.push({ profile, status: "absent" });
		continue;
	}
	const removed = runOmp(profile, ["uninstall", pluginName, "--scope", "user"]);
	if (removed.status !== 0)
		throw new Error(`OMP plugin uninstall failed for ${profile}: ${(removed.stderr || removed.stdout || "unknown error").trim()}`);
	results.push({ profile, status: "uninstalled", output: removed.stdout.trim() });
}

process.stdout.write(`${JSON.stringify({ schema: "omp-vibe-kit-cli-uninstall@1", decision: "pass", results }, null, 2)}\n`);
